-- Hunt Wet AI Database Migration
-- Created: 2024-10-04
...
-- Policy: Users can view their own hunt logs
CREATE POLICY "users_view_own_hunts" ON hunt_logs
  FOR SELECT
  USING (auth.uid() = user_id);

-- Policy: Users can insert their own hunt logs
CREATE POLICY "users_insert_own_hunts" ON hunt_logs
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Policy: Users can update their own hunt logs
CREATE POLICY "users_update_own_hunts" ON hunt_logs
  FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- Policy: Users can delete their own hunt logs
CREATE POLICY "users_delete_own_hunts" ON hunt_logs
  FOR DELETE
  USING (auth.uid() = user_id);
...
GRANT EXECUTE ON FUNCTION get_success_rate_by_pressure(UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_success_rate_by_moon_phase(UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION find_similar_hunts(UUID, TEXT, DECIMAL, TEXT, TEXT, INTEGER) TO authenticated;
